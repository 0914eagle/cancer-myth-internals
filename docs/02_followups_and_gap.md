...
**이 표는 "모델이 안다"의 증거가 아니다.** 첫 열은 적중률, 둘째 열의 여집합은 오탐률이다. 모든 전제에 "거짓"이라 답하는 모델은 100 / 0이 나온다. Qwen2.5-7B의 96 / 15.5는 거기서 조금 벗어난 것뿐이다. "안다"는 참과 거짓을 **가르는 능력**이고, 그건 두 열의 차이에서만 나온다. 신호탐지 이론으로 환산(등분산, AUROC = Φ(d′/√2)):

| 모델 | 적중 | 오탐 | d′ | **언어 판정 AUROC** |
|---|---|---|---|---|
| Qwen2.5-7B-Instruct | 96 | 84.5 | 0.74 | **0.70** |
| Llama-3-8B-Instruct | 97 | 77.6 | 1.12 | 0.79 |
| gemma-4-E4B | 98 | 76.7 | 1.32 | 0.83 |
