...
## 16. 기존 CoT +1 40개 감사와 다음 확인 절차 (2026-09-14)

원래 128토큰 검토문 실행에서 Sol +1을 받은 FPQ 40개를 읽고,
[요약](reviews/cot_positive40_2026-09-14/summary.md),
[질문·참조·검토문·답변·판정 근거 전체](reviews/cot_positive40_2026-09-14/detailed_review.md),
[구조화 분류](reviews/cot_positive40_2026-09-14/annotations.json),
[SHA manifest](reviews/cot_positive40_2026-09-14/manifest.json)를 보존했다.
입력 ID·질문·set, 답변 해시·generation run hash를 대조했지만 전체 run manifest는 미확보다.
1024토큰으로 생성한 전체 FPQ 성능이나 새 PCR을 보고한 것이 아니다.
...
19개만 정답이라는 뜻도, 18개가 확정 주석 오류라는 뜻도 아니다.
18/40은 CoT 성공에 조건화한 탐색 표본의 표시 비율이다. 기존 PCR 40/117=34.2%를
그대로 두고 목표 전제 교정과 추가 의학적 오류를 분리한다. 세 방법의 +1 합계 116은
중복 질문을 포함하는 답변 레코드 수이지 116개 독립 질문이 아니다.

[후속 절차](reviews/fpq_attribution_audit_protocol_2026-09-14.md)는 질문만으로 전제를 먼저
기록한 뒤 참조 범위를 확인하고, 같은 질문의 Plain·FP Identification·CoT를 비교하도록 한다.
이미 본 18개를 가려 재검토해도 사후 분석이며 사전 등록 검증이 되지는 않는다.
참조·전제 귀속 보류는 별도 상태로 남기고 원래 점수에 자동 반영하지 않는다.
추가 의학적 오류는 결과와 무관하게 고정한 질문 표본에서 세 방법을 비교하는 별도 축이다.
새 표본 추출·채점·GPU 생성·test 사용은 이번 문서화에서 실행하지 않았다.

### 16.1 외부 AI와의 독립 검토·근거 대조

사용자는 의료 전문가가 아니므로 사용자 확인을 임상 검증으로 취급하지 않는다.
사용자 요청에 따라 [18개 외부 AI 검토 팩](reviews/external_ai_attribution_18_v1/README.md)을 준비했다.
질문-only 파일을 먼저 보내고 응답을 보존한 뒤 참조 파일을 보낸다. R01–R18로 원 ID를
가리고 기존 CoT·방법명·점수·AI 분류·선택 사유는 공유 파일에서 제외했다.
원문 보존, 원 ID 대응, source 및 공유 파일 SHA는 내부 기록에 남겼다.

외부 AI가 반환한 JSON과 짧은 근거를 받아 누락·인용·범위 해석과 실제 1차 자료를 확인한다.
기존 내 판단과 불일치하면 근거에 따라 수정하되, AI 간 합의를 전문가 gold label로 쓰지 않는다.
임상 불확실성은 보류로 남긴다. 같은 모델 계열의 상관된 오류와 사후 선택 표본의 한계도 유지한다.
현재는 파일 준비만 완료했으며 외부 AI 회신·새 합의 결과·새 점수는 없다.

### 16.2 외부 AI 회신 수령과 근거 대조

이후 `de1829c`에서 외부 AI의 두 응답을 받아
[18개 대조표·출처 확인·정정 기록](reviews/external_ai_attribution_18_v1/followup_2026-09-14/report.md)을 작성했다.
1단계 SHA는 2단계에 기록된 값과 일치했다. 99개 인용 중 98개는 원문 부분문자열과
정확히 일치했고 1개는 공백 축약만 있었다. 해시 자체는 참조 열람 전 작성이라는 시간 순서를
증명하지 않으며 검토자의 사전 노출 공개를 유지한다.

외부 문항 판정은 NOT_CONFIRMED 5 / REVIEW 13 / CONFIRMED 0이지만,
내부 23개 claim은 환자 목표·참조의 반박 설명·추가 약한 전제를 섞고 있었다.
의학 상태도 요약과 달리 전부 UNVERIFIED는 아니었고, 출처 없는 PARTLY_SUPPORTED 2건이 있었다.
원 응답은 수정하지 않고 기계적 점검 결과를 별도 JSON에 기록했다.

본 세션에서 15개 1차 자료 페이지를 직접 열어 주장별 근거와 한계를 기록했다.
R02의 미국 소아 백혈병 발생률 상승, R16의 선별검사 근거 한계 등을 확인했지만,
R05의 확진 가정이나 R13의 개인 예후는 자료 하나로 거짓 전제를 확정할 수 없다.
R10·R17도 외부 AI의 확정적 해석에 대안을 남겼다. 내 이전 R09·R13 표적 요약이 원 참조보다
강했던 점도 정정했다. 원문 보존용 40개 파일은 변경하지 않고
[정정 포인터](reviews/cot_positive40_2026-09-14/ERRATA.md)를 추가했다.
