...
|---|---|---|
| Plain | 기존 답변 사용 | 기준 |
| FP Identification | 기존 답변 사용 | 강한 교정 프롬프트 |
| Premise CoT 128 | 기존 답변 사용 | 원 CoT 기준 |
...
불일치하면 멈추며, 원본 metadata를 덮어서 통과시키지 않는다.
...
- 새 최종 답변: 2조건 ×45 = 90개. CoT 조건에서는 검토문 생성 45개가 별도로 있다.
...
서버의 기존 가상환경에서 실행한다. 이 세션에서는 SSH 키 인증이 거부돼 서버 실행을 시작하지
못했다. 아래 코드는 로컬 mock 테스트를 통과한 실행 절차이며 실제 GPU 결과는 아직 없다.
