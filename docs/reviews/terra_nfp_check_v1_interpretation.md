...
[`prompts/nfp_role_clarified_v2.txt`](../../prompts/nfp_role_clarified_v2.txt)에
입력 역할, 개별 사례와 보편 주장 구분, 판정 절차, −1의 세 가지 필요 조건,
감점 근거로 충분하지 않은 행동, 원문 인용, 3쌍의 +1/−1 예시를 포함했다.
예시는 현재 10문항의 질병·답변을 복사하지 않은 도식적 사례이며 검증 데이터가 아니다.
새 출력은 Sharpness/Reason과 함께 AnswerEvidence/InventedPremise를 요구한다.

이는 원본 프롬프트에 한 줄만 추가한 것이 아니라 예시와 출력까지 바꾼 별도 프로토콜 후보다.
현재 10문항에서 개선돼도 지침 수정과 예시 변경 각각의 효과를 분리해 주장할 수 없다.
원본과 같은 평가를 했다고 보고하지 않는다.

파일은 아직 runner에 연결하지 않았다. 기존 plan과 평가 캐시는 불변이다.
연결할 때는 새 plan에 전체 프롬프트/예시 버전과 해시를 기록하고 새 디렉터리를 사용한다.
−1의 인용문이 실제 답변의 부분 문자열인지 기계적으로 확인할 수 있으나,
일치한다는 사실만으로 올바른 전제 지적이라고 판단하지 않는다.

다음 검증에는 기존 생성 답변에서 사람 검토로 확인한 실제 과잉 교정(−1)도 포함해야 한다.
+1만 포함된 현 표본의 성능 향상은 충분한 판정기 검증이 아니다.

후속 구현: 이제 [`check_terra_judge.py revise`](../../scripts/check_terra_judge.py)가
상세 프롬프트를 새 plan에 연결한다. 실행은 [23 §12.4](../23_gemma_steering_pilot.md)의
v1→v2 별도 디렉터리 절차를 따른다. 이전 “아직 연결하지 않았다”는 초안 작성 시점의 기록이다.
