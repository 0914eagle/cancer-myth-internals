...
발표 편집(2026-09-23): 사용자와 논의해 표 D5는 SFT 세 조건만 제시한다. DPO의 선호쌍 품질과 학습·생성 설정의 적절성은 추가 점검이 필요하므로 발표의 결론 근거에서 제외했다. DPO의 원 점수는 `lora_scores.csv`·`lora_summary.json` 및 Git 이력에 그대로 보존하며 무효 결과로 삭제한 것은 아니다. SFT에도 생성된 수정 질문의 타당성 한계가 있으므로 현재 설정의 탐색 결과로만 설명한다.
