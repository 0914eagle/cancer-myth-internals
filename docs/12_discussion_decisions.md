...
> 최신 후속 결정: [§16](#16-2026-09-15--실험-이력과-새-방법-후보-정리), 통합 수치·변경 이유는 [26](26_experiment_history_and_decisions.md).
...
## 2026-09-09 후속 반영 — 질문별 상세 설명을 발표 원고 안에 통합

사용자가 실제 읽는 [18 — 발표 원고](18_professor_presentation_script.md)의 각 장에 상세 설명을 직접 추가했다. 별도 문서 링크만 따라가야 하는 구성을 피하고, 첫 화면 요약과 질문별 바로가기를 함께 제공한다. 기존 23장과 Table 1·2·3의 행·열은 유지한다.

- Table 1 일곱 신호의 입력·학습·판정 시점을 풀어 썼다. 학습된 출력 readout은 Plain 답변의 평균 token log probability와 첫 token entropy를 읽는 질문 라벨 분류기이며, 답변 의미·CoT를 읽거나 LLM을 fine-tuning하는 방법과 구분한다.
- 내부 DiM의 FPQ/정상 평균 차와 교정 C의 같은 질문·다른 응답 평균 차를 구분했다. 전제 추출·사실 확인과 CoT 검토·별도 모니터는 환자 질문부터 최종 선택까지 예시로 설명했다.
- TPR·FPR·ACC·AUROC·개입률을 가상 혼동행렬로 설명하고, dev 문턱과 test 실제 FPR, 탐지 τ_det와 최종 τ_policy를 구분했다. 가상 숫자는 실험 결과가 아니다.
- FPQ/PCR/PCS/NFP/TPQ/QA 지표, Well 수동 주석과 원 split 문항 ID, GEPA·Monitor·MDAgents 및 Well 세 전제 처리 방법의 차이를 해당 장에 넣었다.
- Table 2의 아홉 방법과 선정 이유, Table 3(a)의 같은 C·같은 K와 3(b)의 같은 문항 목록을 설명했다. Task 2의 정상 전제 보존과 Task 3의 의료 QA 정답 성능은 같은 표에 실어도 별개의 평가다.
- CAST의 조건·행동 분리는 선행 구조이고, Two Axes와 Gated는 의료 데이터에 재학습해 적용할 직접 비교 대상이다. 데이터 변경만으로 새로운 알고리즘을 주장하지 않으며, CoT의 faithfulness와 탐지 효용도 구분한다.

이번 반영은 설명 확장이다. 본 실험 수치·재현 완료 주장·허용폭 확정·새 알고리즘 변경을 추가하지 않았다. 미확정 실행 설정은 13/19의 현재 명세와 함께 읽는다.

## 2026-09-09 후속 반영 — Method 네 장의 절차와 발표 원고 보강

[18의 Method 14–17장](18_professor_presentation_script.md#method-input)을 각 장별로 확장했다. 기존 네 장과 실행 명세의 설정을 유지하면서 입력·산출물·연산·선택 기준을 해당 장 안에서 설명한다.

- 14장: 질문과 라벨의 분리, chat template과 마지막 유효 토큰, hidden tensor에서 n×d 행렬을 얻는 과정, grouped fit/dev/held-out 역할과 저장 자료.
- 15장: 표준화, 가중 이진 교차엔트로피와 L2의 의미, 가상 클래스 가중치 계산, LLM 고정과 probe 학습의 차이, 층 선택 및 τ_det/τ_policy 구분.
- 16장: 같은 fit 질문의 +1/−1 응답 쌍, 타깃 모델 teacher-forcing, 토큰 평균과 질문별 차이 평균, 단위 방향의 가상 계산, 출처·길이 교란 및 raw residual 공간.
...
## 13. 2026-09-10 문서 세션 추가 논의 — 방법 변형·ARR·GPU

세부는 [22](22_method_variants_and_arr_plan.md). 여기에는 결정만 적는다.
...
## 16. 2026-09-15 — 실험 이력과 새 방법 후보 정리

- 결과 나열보다 **관측 수치 → 해석 가능 범위 → 중단 이유 → 다음 변경**을 우선해 [26](26_experiment_history_and_decisions.md)에 통합했다. 원본 보고서와 사용자 콘솔을 구분하고 Quick45/overnight 보고서 사본·해시를 보존했다.
- α=0 동일 답변의 NFP 판정 5/30 뒤집힘은 판정 점검의 이유, Quick45의 구제 0/26은 해당 C 설정의 한계, Qwen final 512의 다수 잘림은 1024 전환의 이유다. 이를 각각 방법 전체의 실패로 부르지 않는다.
- Gemma 초기 성공/실패 참조 후보 232와 실제 overnight fit 완전 쌍 135는 다른 범위다. 자기 모델 64쌍은 지시 차이이며 검증된 교정 성공 쌍이 아니다.
- 장기 표는 질문 판별 / 교정·정상 보존·의료 QA / CREPE 일반화·전이 후보로 정리한다. 실제 suite 구현과 장기 표 전체 완료를 구분한다. Well 원 split의 정확 재현이나 원 논문 숫자 재사용을 주장하지 않는다.
- [27](27_method_candidates_and_tests.md)에 선택 신호·학습 C·틀린 반박 억제·정상 보존·반사실 부분공간·디코딩·head/SAE 후보를 정리했다. 기존 선행과 같은 수식은 신규성 근거로 쓰지 않고, 부호 보존 증폭의 정상 질문 보장은 철회한다.
- 검증된 쌍둥이 데이터가 이미 전부 확보됐다고 가정하지 않는다. 후보 검토와 자료 준비·실행 승인을 구분한다.
- **사용자 실행 선호 확정:** 현재 baseline은 유지. 다음 독립 실험부터 물리 GPU 0·1 병렬을 기본으로 설계하고 CPU 분석·예산 제한 Terra 판정을 병행한다. 의존 작업만 순차로 둔다. 자동 무제한 판정, 다른 GPU 사용, 로컬 torch/pytest 실행은 하지 않는다.
- 이번 변경은 문서와 보고서 사본뿐이다. 실행 중 코드·프로세스·평가 원점수는 바꾸지 않았다.

## 17. 2026-09-15 — 10월 12일 ARR 제출 확정: 분석 + 방법 한 개

사용자 결정. 절차·근거는 [22 §8–9](22_method_variants_and_arr_plan.md), 현재 상태는 [26](26_experiment_history_and_decisions.md)·[28](28_advisor_progress_narrative.md). (앞선 커밋 5ef25a7의 12 기록은 §16 제목 충돌로 누락됐고 README에만 남았다. 여기로 옮긴다.)

### 17.1 분석 부분

- **주장:** 선택이 시소를 얼마나 푸는가. oracle 상한(Gemma dev 59/100 대 CoT 34/100), 배포 가능한 탐지기의 한계, 텍스트·내부 신호의 상보성, 평가 감사(판정기 역할 혼동·전제 귀속·출력 길이).
- **들어가는 것:** Qwen 732문항 5방법 Well 0–5 + Plain·FP·CoT의 원 PCR, Table 1 crossfit 판별(text/hidden/DiM/direct/review, 같은 오탐의 TPR, 겹침), 부록 A1 게이트 × FP 라우팅과 harm 귀속, 평가 감사 절, steering 음성 결과 절(Quick45 0/26, S와 생성의 분리, α=.6 퇴화), Gemma pilot 탐색 절.
- **빠지는 것:** CREPE, 일반 의료 QA(미실행 표기), 임상의 평가(확보 못 하면 독립 판정자 2인으로 대체하고 한계에 명시).

### 17.2 방법 부분 (사용자 결정: 방법을 제안한다)

**선택 기준:** 새 생성이 거의 없고, 진단된 실패를 겨냥하며, 같은 조건 ablation이 있을 것.

**채택: A + B를 하나의 방법으로. "개입 효용으로 학습하고 텍스트·내부 신호를 결합한 선택."**
- A 상보적 선택: 텍스트 점수와 DiM 점수를 같은 오탐에서 결합해 FP Identification으로 라우팅. 근거는 Qwen dev에서 DiM만 잡은 FPQ 21개.
- B 결과 지도 게이트(27의 S6): 라벨을 전제 진위가 아니라 "FP 개입이 이 문항을 구제하는가 / 정상 질문을 망치는가"로 두고 학습. "개입 안 함"을 정식 행동으로. fit 문항도 732 suite에 포함되므로 FP 답변과 판정이 곧 라벨.
- 둘 다 732 suite 판정 후 CPU만으로 실행. 라우팅 행은 저장 답변 조합이라 재판정 없음.

**ablation (같은 오탐, 같은 FP 답변):** 텍스트 단독 / DiM 단독 / 결합 × 전제 라벨 / 효용 라벨, random, oracle. 부록 A1 격자에 행 추가.

**주장 문장:** 개입 효용으로 학습하고 텍스트·내부 신호를 결합한 선택은 같은 정상 질문 피해에서 oracle 상한의 X%를 회수하며 단일 신호 게이트와 CoT를 넘는다. X가 작으면 그대로 보고한다.

**선행과 차이:** Two Axes(전제 라벨 단일 probe 라우팅) 대비 라벨·신호 결합·정상 피해 운영점 셋. 안전 쪽 gated steering(SafeSwitch, GSAE, BarrierSteer)과는 과제가 다르며 알고리즘 신규성은 주장하지 않는다. 임상 답변 선택 라우터 [Route, Don't Fix](https://arxiv.org/html/2609.14825)는 확인 후 인용.

**제외:** 27의 C2–C6(검증된 쌍 C)은 판정 예산과 겹치고 null 위험이 커서 10월 원고에서 뺀다. A+B가 9/30 전에 끝나고 예산이 남으면 dev 소규모로만 붙인다. 쌍둥이 증폭(T1/T2)도 다음 논문.

### 17.3 예산·일정·오늘 정할 것

- **예산:** fit 라벨링 별도 작업과 C2는 10월 이후. 판정 예산은 732 채점에 집중. GPU 1은 crossfit·게이트.
- **일정:** 9/19 생성 완료·`status`·1024 확인·crossfit·judge-plan·채점 시작 → 9/24 표 채움·Qwen 게이트 replay·paired bootstrap → **9/25 프레이밍 결정** → 9/26–30 A+B(CPU) → 10/3 초고 → 10/8 모의 리뷰·교수님 열람 → 10/12 제출.
- **오늘 정할 것:** 임상 훈련자 확보 여부(22 §9 11번). 방법별 답변 150–200개(rescue·harm 전부 포함)를 blinded로 읽어 교정의 의학적 정확성을 확인할 사람. 없으면 판정기 2종으로 대체하고 한계에 적는다.

### 17.4 문체 교란 검정을 셋 다 돌린다 (2026-09-16)

crossfit에서 text 0.755 > hidden 0.693이 나왔고, 실험 세션은 "문체 때문일 것"이라 보았다. 사용자 결정:
**세 검정(문체 하한선·의역 통제·참 전제 쌍둥이)을 모두 돌려 확정하고 넘어간다.** 절차·판독 기준·명령은
[29](29_style_confound_tests.md). 판정 배치와 독립이라 병렬로 돌리고, 결과는 9/25 프레이밍 결정 전에 29 §7에 채운다.
어느 결과든 17.2의 방법 주장은 유지되고 주 게이트를 어느 신호로 부르느냐만 바뀐다.
...
codex 대신 Claude Code CLI(`claude -p`)를 쓸 수 있게 `--backend claude`를 열었다. 판정기는 Terra 그대로.
