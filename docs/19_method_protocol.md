...
> **현재 첫 실행은 [23](23_gemma_steering_pilot.md).** 아래는 본 실험의 이전 설계이며,
> 지금은 Gemma 한 모델의 baseline·기본 C 개입을 먼저 검증한다. 이 파일의 두 모델·nested
> cross-fitting·probe gate를 첫 파일럿에서 모두 실행하는 것은 아니다.
