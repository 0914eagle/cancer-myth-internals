...
**표 번호 (2026-09-09 확정).** Task와 Table을 일대일로 맞춘다. Task 1 → Table 1 (탐지 AUROC). Task 2 → Table 2 (본 표: 방법 × PCR/PCS/NFP/TPQ + 의료 QA 3열) + Table 3 (조건 × 개입 격자). Task 3 → Table 2의 오른쪽 열 셋(MedQA, PubMedQA, Medbullets). Figure 1 = TPQ 대 PCR 산점도. 논문은 "신호가 있다 → 골라서 개입하면 된다 → 전체 표" 순서로 읽히고, 본 결과는 초록·서론에서 미리 보인다. 논문에 싣는 형태(비용·발화율·CI 제외)는 [12 §11](12_discussion_decisions.md) 끝의 "논문용 표" 참조.
