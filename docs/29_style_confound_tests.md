...
1. **의역은 출처 지문을 지우지만 게이트는 살아남는다.** natural→para에서 text 0.694, hidden 0.694로 natural 안(0.75/0.72)에서
   ...
   (믿음 서술 틀 + 통념 주제)이고, 쌍둥이 편집이 바꾸는 것(진위)이 아니다. 7.2 판독 1을 한 단계 더 좁힌다:
   natural 0.75 ≈ 출처 지문(≈0.05, 의역으로 사라짐) + 틀·주제(≈0.20, 의역에서도 남음) + 진위(≈0, 쌍둥이에서 0).
2. **§5 사전 표의 다섯째 행("natural→para ≪ para")은 성립하지 않는다**(0.694 vs 0.700). 그 행은 의역이 전제 외 신호를 모두
   지운다는 가정 위에 있었는데, 의역은 설계상 믿음과 주제를 그대로 옮기므로 틀·주제 신호를 지우지 못한다. 이건 의역 통제의
...
**최종 판독 (§5 표 대비).** 첫 행(전반이 출처)이 가장 가깝되, "출처"를 "출처 지문 + 틀·주제"로 읽어야 한다. 12 §17.2의
