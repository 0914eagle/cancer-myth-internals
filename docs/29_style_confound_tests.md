...
판정 배치(2,965회; 9/16 22:40 기준 미시작, 30 참조)와 독립이므로 병렬로 돌린다. 1은 지금, 2·3은 이번 주.
...
1. **natural에서 학습한 게이트는 어느 신호든 전제를 읽지 않았다.** natural→twins, natural→edited가 text·hidden·mean 모두
   ...
   "NFP에 몰린 암종 어휘와 더 긴 길이"를 배운 것이고(9/18 확인, §7.5), §5 표의 첫 행("게이트 전반이 출처를 읽는다")이 text에도 그대로 적용된다.
...
4. **DiM(mean)은 문체를 고정하면 무너진다.** natural 0.687 → twins 0.627 → edited 0.606, 하한선 아래. FPQ−NFP 평균차
   ...
   다시 확인해야 한다.
5. **문체 차이의 실체는 생성 모델 지문이 아니라 주제·길이다(9/18 정정: 두 집단의 서술 틀은 같다, §7.5).** same_writer(GPT-4o가 쓴 NFP 31개만 음성)에서 style 0.814,
...
### 7.3 검정 2 (의역): `style_controls/v1`, 2026-09-16 밤

para 조건: 한 writer(Sonnet 5, p3)가 FPQ 539 + NFP 135를 같은 두 문장 틀로 다시 쓴 것. 의역은 원본의 fold를 물려받는다.
(v1에서 쌍둥이의 hidden/mean 행은 드라이버의 id 재명명 버그로 특성 조회에 실패해 건너뛰었다. 7.2의 v0_twins 값이 그 행의
정본이고, 버그는 고쳤다(`remap_twins`). 아래 표의 natural·same_writer·전이 행은 v0와 같은 수치가 재현됐다.)

| 조건 (pos/neg) | text | style | masked | hidden | mean | hidden − text [CI] |
|---|---|---|---|---|---|---|
| natural (583/149) | 0.748 | 0.687 | 0.703 | 0.720 | 0.687 | −0.028 [−0.067, +0.006] |
| para (539/135) | 0.700 | 0.621 | 0.628 | 0.656 | 0.656 | −0.044 [−0.089, −0.002] |
| natural → para | 0.694 | 0.593 | 0.603 | 0.694 | 0.662 | −0.000 [−0.054, +0.046] |
| para → natural | 0.709 | 0.643 | 0.646 | 0.693 | 0.688 | −0.016 [−0.065, +0.033] |

**판독.**

1. **의역은 출처 지문을 지우지만 게이트는 살아남는다.** natural→para에서 text 0.694, hidden 0.694로 natural 안(0.75/0.72)에서
   0.05 정도만 빠진다. 반면 같은 게이트가 natural→twins에서는 0.49다(7.2). 즉 natural 게이트가 읽는 것은 의역이 보존하는 것
   (NFP의 암종 어휘 + 길이)이고, 쌍둥이 편집이 바꾸는 것(진위)이 아니다. 7.2 판독 1을 한 단계 더 좁힌다:
   natural 0.75 ≈ 출처 지문(≈0.05, 의역으로 사라짐) + 주제 분포·길이(≈0.20, 의역에서도 남음) + 진위(≈0, 쌍둥이에서 0).
2. **§5 사전 표의 다섯째 행("natural→para ≪ para")은 성립하지 않는다**(0.694 vs 0.700). 그 행은 의역이 전제 외 신호를 모두
   지운다는 가정 위에 있었는데, 의역은 설계상 믿음과 주제를 그대로 옮기므로 주제 분포·길이 신호를 지우지 못한다. 이건 의역 통제의
   한계이지 게이트의 증거가 아니다. 진위 여부는 쌍둥이 전이(7.2)만이 가른다.
3. **para 안에서 hidden은 text 아래다** (−0.044 [−0.089, −0.002]). style/masked 하한선 0.63 위로 text +0.07, hidden +0.03.
   어느 조건에서도 hidden이 text를 넘는 곳은 없다. 7.2 판독 2·3과 합치면, 마지막 토큰 잔차는 표면 단어가 주는 것 이상을
   보여주지 못했다.
4. **para→natural 0.709/0.693.** 한 writer 문체로 학습한 게이트가 natural로 돌아와도 비슷하다. 1번의 대칭 확인.

**배포 함의.** natural 게이트는 "통념 주제에 대해 믿음을 서술하는 질문" 탐지기다. 같은 주제를 참 믿음으로 서술한 사용자
(쌍둥이)도 같은 비율로 잡는다(natural→twins 0.49). 라우팅에 쓰면 교정 개입이 참 믿음 사용자에게도 같은 확률로 들어가고,
그 harm은 natural NFP(다른 주제, 다른 틀)로는 측정되지 않는다. 쌍둥이 음성이 harm 측정에 들어가야 한다.

**v2 (2026-09-17 아침, 쌍둥이 hidden 행 복구 + 진위 학습 게이트의 배포 분포 평가).** natural·twins·edited·para·same_writer와
모든 전이 행이 v0/v1과 같은 수치로 재현됐다(twins hidden 0.754, edited hidden 0.764 복구). 새 두 행:

| 학습 → 평가 (pos/neg) | text | style | masked | hidden | mean | hidden − text [CI] |
|---|---|---|---|---|---|---|
| twins → para (539/135) | 0.536 | 0.546 | 0.591 | 0.600 | 0.611 | +0.063 [−0.003, +0.134] |
| edited → para (539/135) | 0.516 | 0.498 | 0.523 | 0.536 | 0.495 | +0.021 [−0.042, +0.079] |

진위로 학습한 게이트는 한 writer 분포에서도 작동하지 않는다. 선택지 (a)("쌍으로 학습해 배포")는 닫혔다. 유일하게 눈에 띄는
것은 twins→para·twins→natural에서 내부 신호(hidden 0.60, mean 0.61–0.63)만 우연 위이고 text는 우연 이하(0.46–0.54)라는 점이다.
진위 쌍으로 학습했을 때 text가 배우는 것(단정어)은 다른 분포에서 뒤집히고, 내부 신호가 배우는 것은 조금 옮겨간다. 차이의 CI가
0에 걸쳐 있어 주장으로 쓰지는 않고, 31에서 "안다" 집합으로 갈라 다시 볼 후보로만 둔다.

**추가 조건(v2에서 실행됨).** 진위로 학습한 게이트(twins, edited)를 para에 평가하는 `twins->para`, `edited->para`를 드라이버에
넣었다. twins->natural이 0.456이었으므로 기대는 낮지만, "쌍으로 학습한 게이트를 배포 분포에서 평가"라는 선택지 (a)의
가부를 직접 답하는 행이다. 쌍둥이 hidden 행 복구와 함께 v2로 한 번 더 돌린다(CPU 1–3시간).

**최종 판독 (§5 표 대비).** 첫 행(전반이 출처)이 가장 가깝되, "출처"를 "출처 지문 + 주제 분포·길이"로 읽어야 한다. 12 §17.2의
A(text+DiM 결합)는 근거를 잃었고, DiM은 7.2 판독 4로 접는다. 게이트를 논문에 남기려면 (i) 쌍둥이 전이 0.49와 harm을
그대로 보고하는 "주제 탐지기"로서, 또는 (ii) 31의 지식/순응 분해 위에서 "모델이 아는데 순응하는 경우"를 가르는 신호로서만
가능하다. (ii)가 다음 실험이다.

### 7.5 게이트가 실제로 읽는 것 (`scripts/inspect_gate_features.py`, 2026-09-18)

전 732문항에 text(TF-IDF)와 style 분류기를 맞춰 상위 가중치를 찍었다. **두 집단의 서술 틀은 같다.** NFP도 "Since my brother was
diagnosed with ..., we've been on edge, fearing that ... What can we do?"처럼 상황 + 믿음 + 질문 구조이고, 1인칭 73%(FPQ 86%),
물음표 1개, 오히려 더 길다(단어 중앙값 50 vs 40, 문장 3 vs 2). 이전 판의 "FPQ 서술 틀 대 정상 질문 틀"은 틀렸다.

| 쪽 | 상위 특징 | 뜻 |
|---|---|---|
| NFP | tumor, germ cell, hodgkin, amputation, chemotherapy, radiation, oncologist, "the doctor mentioned", his/him | 특정 암종·치료 어휘. NFP가 백혈병·생식세포종양·호지킨 등 소수 암종에 몰려 있음(nfp_1000~1004가 전부 백혈병·생식세포종양) |
| NFP | n_chars 335 vs 279, n_sentences 3.2 vs 2.7, n_digits | 길이 |
| FPQ | cancer, family, risk, genetic, should, given that | 일반어. 통념이 일반 진술이라 특정 암종 어휘가 적음 |

**정정된 판독.** natural 게이트(text 0.75, hidden 0.72, style 0.69)가 읽는 것은 **NFP의 주제 분포(암종)와 길이**다. 진위(쌍둥이
0.50)도, 저자(의역 0.69)도, 틀(같음)도 아니다. 이는 NFP가 어떤 암종에서 얼마나 길게 생성됐느냐는 표집 성질이고, 게이트 학습·평가에
쓰면 그대로 지름길이 된다. 문서 전체의 "틀·주제"는 "주제 분포·길이"로 읽는다.
