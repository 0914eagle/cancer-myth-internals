...
**차별점 문장 (갱신).** 조건부 steering(조건·행동 벡터 분리)도, probe 게이트 프롬프트도, 의료 게이트 ITI도 있다. 우리는 조건부 steering의 확장이고, 다른 것은 (a) 조건 신호가 **배경화된 의료 전제의 진위**이고 개입 대상이 교정/비교정 대조 방향이라는 것, (b) 통제군이 **어려운 정상 질문 통제군 NFP**라는 것, (c) 같은 게이트 아래에서 프롬프트 라우팅(A1, 교정 품질 47–61%)과 활성값 개입을 비교한다는 것, (d) Cancer-Myth 지표로 고정 held-out 비교표를 낸다는 것 (원본 Table 1과는 평가 범위가 달라 참고로 분리).
