...
> **2026-09-08 후속 정리.** 선행연구의 현재 해석은 [10](10_evidence_and_baselines.md)과 [11](11_tripathi_review.md)을 우선한다. 아래 이전 검토에서 “비어 있음/표면상 구분 불가능/내부에서만 읽힘” 등은 독창성이나 불가능성의 확정 근거가 아니다.
...
- *교정 품질*: challenge 출력에서 **전체 거짓 전제 문항** 중 NLI 검증 교정 비율은 47% (Llama) / 61% (Qwen). gated 문항의 조건부 성공률이 아니다. NLI 미통과에는 실제 오류와 판정의 모호성이 섞일 수 있어 전부 틀린 교정이라고 세지 않는다. [원문 §6](https://arxiv.org/html/2607.08456v1#S6), 현재 해석은 [17](17_manuscript_storyline.md).
...
| 개입 | 프롬프트 라우팅 | 내부 방향 (C) |
| challenge 출력의 전체 FPQ 중 NLI 검증 교정 비율 | 47–61%, gated 조건부 비율 아님 | 우리 판정기로 측정 필요 |
...
2. 실험 1 기대치: 마지막 토큰 probe **0.70 안팎**이 일반 도메인 기준선. 의료 전제는 의사 검증이라 더 깨끗할 수 있고, 전제 구간 위치는 더 높을 수 있다. 다른 데이터의 AUROC를 의료 실험의 합격선으로 쓰지 않는다.
3. difference-of-means 방향이 logistic probe보다 높았다 (0.74–0.78 vs 0.69–0.73). 우리도 둘 다.
4. 통제 프로토콜: 4-way split (probe-train / tune / certify / test), bag-of-words 기준선, 100 resplit 감사. 그대로 쓴다.

### A2. Tripathi — 2608.23666 (2026-09-08 정정)

원문과 공개 코드 확인은 [11](11_tripathi_review.md)에 통합했다. 이전 절의 **“표면 압력 gate라 Cancer-Myth/NFP를 못 가른다”는 설명을 철회한다.** 저자 runtime은 EHR·질문·이전 응답을 입력으로 만든 뒤 타깃 모델의 hidden state에 선형 probe를 적용한다. 공개 artifacts 및 추론 코드가 있으며 정확한 재학습·평가 재현과는 구분한다.

이 방법은 의료 내부 gate와 head steering의 직접 선행연구다. 데이터·목표를 바꾸어 적용할 수 있지만, 배경 전제에서의 성능은 실험할 사항이다. 현재 연구계획과 기여의 경계는 [09](09_research_proposal.md)를 따른다.
...
| 판정시키면 오탐 | Well-Actually negative bias | **A1이 반대 방향(accommodation)도 확인.** 시험된 elicitation의 한계이며 모든 prompting의 한계는 아님 |
| probe 게이트 × 개입 | 비어 있음 | **A1이 프롬프트 라우팅으로 함** (일반 도메인). 내부 개입 게이트는 남음 |
| 의료 + 게이트 + ITI steering | 비어 있음 | **A2가 함** (EHR 맥락, 내부 probe gate; 11의 코드 확인 참고) |
...
**차별점 문장 (갱신).** 조건부 steering(조건·행동 벡터 분리)도, probe 게이트 프롬프트도, 의료 게이트 ITI도 있다. 우리는 조건부 steering의 확장이고, 다른 것은 (a) 조건 신호가 **배경화된 의료 전제의 진위**이고 개입 대상이 교정/비교정 대조 방향이라는 것, (b) 통제군이 **어려운 정상 질문 통제군 NFP**라는 것, (c) 같은 게이트 아래에서 프롬프트 라우팅(A1)과 활성값 개입을 비교한다는 것, (d) Cancer-Myth 지표로 고정 held-out 비교표를 낸다는 것 (원본 Table 1과는 평가 범위가 달라 참고로 분리).
