...
Cancer-Myth FPQ 585개와 NFP/TPQ 150개에서, 전제 구간·질문 끝·마지막 프롬프트 토큰의 hidden state가 "전제가 거짓이다"를 갈라내는가. **Well의 TPQ는 NFP 150개 질문에 참 전제 주석을 더한 것이므로, 명목상 고유 질문은 735개이며 정상 질문은 300개가 아니다.** 갈라낸다면 그 방향(A)과, 응답이 전제를 고칠지 따를지를 가르는 방향(C)은 얼마나 겹치는가.

이 문서의 기존 실측 표에 남은 TPQ 148은 당시 실행에서 평가한 TPQ 행 수다. NFP 150과 별개인 질문 148개라는 뜻이 아니며, 기존 실측 값은 변경하지 않는다. 현재 재실행에서는 원 질문 ID를 연결한 manifest로 실제 고유 질문 수와 위치별 유효 표본 수를 확인한다. [Well-Actually §3.2](https://arxiv.org/html/2608.06539v1#S3.SS2)
