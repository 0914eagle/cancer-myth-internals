...
**텍스트 상한 결과 (2026-09-08, 재정렬 전 rows).**
...
**그래서 stage 9: 최소 짝.** 각 fpq에서 정렬된 전제 구간만 참 믿음으로 바꿔 끼운 쌍둥이(`set=tpair`, `pair_id=<fpq id>`)를 만든다. 바깥 텍스트는 바이트 단위로 같고, 두 번째 LLM 호출로 거짓 믿음이 사라졌는지 확인한다(`make_true_twins.py`, `run_e0_twins.sh`). fpq vs 쌍둥이로 probe와 텍스트 상한을 다시 잰다(`probe_sweep_twins/`). 여기서 probe가 텍스트를 뚜렷이 넘으면 A는 진위를 읽는 것이고, E2의 게이트는 이 대조로 학습한다. 쌍둥이는 E2에서 "게이트가 참 전제 쌍둥이에 열리면 안 된다"는 가장 엄격한 통제군이기도 하다. 남는 한계: 거짓 믿음은 "only / always / cure" 같은 단정어와 상관돼 텍스트 분류기도 어느 정도 맞힌다. 그래서 절대값이 아니라 probe − 텍스트의 차이를 본다.

**최소 짝 v1 결과 (27B, 2026-09-09).** fpq 403 vs 참 쌍둥이 403: probe 0.986 [0.98, 0.99] (L22 A_premise), 텍스트 0.928, **글자 수만 0.82**. 쌍둥이 구절이 원문보다 길고 부정어("does not", "isn't")가 들어가 텍스트 분류기가 그걸로 맞힌다. probe가 텍스트 위이고 신뢰구간이 안 겹치니 글자 이상의 신호는 있으나, 쌍둥이가 "최소"가 아니어서 주장에 못 쓴다. 재정렬 후 fpq vs NFP는 probe 0.79 / 텍스트 0.67 (차이 0.12).

**v2 (2026-09-09).** 두 가지를 바꿨다. (a) 바꿔 넣는 구절에 길이(±50% 밖이면 버림, 목표는 같은 길이)와 **부정어 금지**(긍정문으로 표현) 제약. (b) 같은 제약으로 **거짓 믿음을 유지한 채 바꿔 쓴 구절**(`set=fpair`, label 1)도 만든다. `fpair vs tpair`는 둘 다 LLM이 같은 규칙으로 쓴 구절이라 작성자·길이·문체가 양쪽에 같고 진위만 다르다. 감사 파일에 부정어 비율과 길이 비가 나온다. 결과는 `probe_sweep_pairs/`(fpair vs tpair)와 `probe_sweep_twins/`(원문 fpq vs tpair) 두 벌.

**stage 8 결과 (27B).** 다른 모델의 +1/−1 답으로 만든 c_pair는 27B 자신의 D 상태에서 PCR을 최고 0.62(L21–22)로만 읽고, 문항별 부호 일치 0.7. 자연 c_e(E→D 전이 0.75)보다 약하다. E2는 자연 C로 가고 c_pair는 보조.
...
- [ ] A 읽기의 최소 짝 통제 — 텍스트 상한 0.77이 B/D probe를 넘어 **필수**. `run_e0_twins.sh` → 모델별 `STAGES=9`
